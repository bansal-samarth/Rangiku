...
const CheckOutPage = () => {
  ...
  const handleDecode = async (result) => {
    ...
    try {
      ...
      // API request to check out visitor
      const response = await axios.put(
        `http://localhost:5000/api/visitors/${visitorId}/check-out`,
        {},
      ...
      if (response.data.visitor) {
        ...
        if (visitor.status === 'checked_out') {
          ...
          setStatus('error');
          setErrorMessage('An error occurred during check-out.');
        }
      } else {
        // API did not return visitor info or had an error
        setStatus('error');
        setErrorMessage(response.data.message || 'An error occurred during check-out.');
      }
    } catch (error) {
      setStatus('error');
      setErrorMessage(error.response?.data?.message || 'An error occurred during check-out.');
  ...
  return (
    <div className="max-w-md mx-auto my-10 p-8 bg-white rounded-xl shadow-xl border border-gray-100">
      <div className="flex items-center justify-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800 ml-3">Visitor Check-out</h1>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-green-50 rounded-lg p-4 text-center">
            ...
            <p className="text-green-600 text-sm mt-1">
              Scan your QR code to check out
            </p>
          </div>
          <button
            onClick={startCamera}
      ...
      {status === 'scanning' && (
        <div className="flex flex-col items-center justify-center p-8">
          ...
          <p className="mt-4 text-green-700 font-medium">Processing check-out...</p>
        ...
        <div className="space-y-6">
          <div className="bg-green-50 border-l-4 border-green-500 p-5 rounded-lg">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="font-medium text-green-800">Check-out successful!</p>
                <p className="text-green-600 text-sm mt-1">Thank you for your visit</p>
              </div>
            </div>
          </div>
          ...
          {visitorInfo && (
            <div className="mt-4 bg-white border border-green-100 rounded-lg shadow-sm p-5">
              <div className="flex items-center mb-4">
                ...
                <h3 className="font-semibold text-green-800 text-lg">Visit Summary</h3>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b border-green-50">
                  <span className="text-green-700 font-medium">Check-out time</span>
                  <span className="text-gray-800">{visitorInfo.check_out_time ? new Date(visitorInfo.check_out_time).toLocaleString() : 'N/A'}</span>
                </div>
